...
# Creating pipeline
s=['pyod_ae','pyod_vae','pyod_cof','pyod_abod','pyod_hbos','pyod_iforest','pyod_lof','pyod_knn','pyod_ocsvm','pyod_loda','pyod_cblof','pyod_sogaal','pyod_mogaal']
for idx, station in enumerate(s):
    ...
    step_4 = PrimitiveStep(
        primitive=index.get_primitive('d3m.primitives.tods.timeseries_processing.transformation.axiswise_scaler'))
    step_4.add_argument(name='inputs', argument_type=ArgumentType.CONTAINER, data_reference=attributes)
    step_4.add_output('produce')
    pipeline_description.add_step(step_4)

    # Step 5: algorithm
    step_5 = PrimitiveStep(primitive=index.get_primitive('d3m.primitives.tods.detection_algorithm.'+s[idx]))
    step_5.add_argument(name='inputs', argument_type=ArgumentType.CONTAINER, data_reference='steps.4.produce')
    step_5.add_output('produce')
    pipeline_description.add_step(step_5)
    ...
    # Output to json
    data = pipeline_description.to_json()
    with open(s[idx]+'.json', 'w') as f: f.write(data)
    print(data)
...
for idx, station in enumerate(s):
    table_path = './examples/sk_examples/metric_detection/periodic_data/periodic_data_0_from2020-11-29to2020-12-06_10457.csv'
    target_index = 2 # which column is the label
    pipeline_path = s[idx]+".json"
    metric = "ALL"
